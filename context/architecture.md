...
**File-granular vs. task-granular expiry data — do not conflate these two shapes:**
- `TaskSummary.fileExpiry` (`TasksService.attachFileExpiry`, `GET /tasks`) is **task-granular** — one summary per task, collapsed to that task's single *most urgent* file if it has several. This powers the normal task-row Tasks table (`expiryFilter === 'all'`) and must stay task-granular there; it deliberately does not represent "how many files," only "what's the worst case for this task."
- `ExpiryFileRow` (`FileAttachmentsService.getExpiringFiles`, `GET /file-attachments/expiring?workspaceId=`) is **file-granular** — one row per `FileAttachment`, no collapsing. This powers the dedicated expiry-review table (`expiryFilter !== 'all'`) so its row count always equals Workspace Status's file-granular `metrics.expiredFiles`/`expiringFiles` count exactly. A task with several expired files produces several rows here, one per file, with the same "Linked Task" value repeated — never hidden inside a single row.
- Both share the same classification logic (`apps/api/src/modules/tasks/file-expiry.util.ts`, `classifyFileExpiry()`) so status/day-math thresholds can't drift between the two views.
- Rule of thumb: **if a UI element claims to count "files," it must be built from `ExpiryFileRow`/`FileAttachment` rows, never from tasks** — this file/task unit mismatch was the exact bug fixed here (Workspace Status said "18 files expired," the old task-collapsed table showed only 3 rows for those 18 files spread across 3 tasks).
